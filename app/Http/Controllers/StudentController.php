    ...
    public function getStudents(Request $request){
        try{
            ...
            $students = Registration::select('registrations.registration_id as studentid','registrations.name','registrations.dob as dob',
                ...
                DB::Raw("IF(users.device_token IS NULL or users.device_token = '', 0, 1) as notificationflag"),'users.status')
                ...
                ->join('users','users.username','registrations.registration_id')->where([['registrations.session',$getcompanyid],['registrations.status','Active']])
    ...
    public function updateStudent(Request $request){
        try{
            ...
            $validator = Validator::make($request->all(),[
                ...
                'attachment' => $request->activitytype=='updatephoto'?'required|mimes:'.implode(',',$acceptFiles['extension-type']).'|max:'.$acceptFiles['max-size'].'':'',
            ]);
    ...
    public function getAttendanceReport(Request $request){
        try{
            ...
            if($reporttype==='datewise'){
                ...
                $result['holiday'] = $holidays->reason;
                return customResponse(1,$result);
